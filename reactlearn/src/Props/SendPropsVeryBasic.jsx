import React from 'react'
import ArrayBasicProps from './ArrayBasicProps';
import user from './user.json'

export default function SendPropsVeryBasic( {baiscProps}) {
    const dummyData = [
        {
          fname: "John",
          lname: "Doe",
          age: 28,
          city: "New York",
          jobprofile: "Software Engineer",
          address: "123 Main St, New York, NY"
        },
        {
          fname: "Jane",
          lname: "Smith",
          age: 34,
          city: "Los Angeles",
          jobprofile: "Graphic Designer",
          address: "456 Elm St, Los Angeles, CA"
        },
        {
          fname: "Michael",
          lname: "Johnson",
          age: 45,
          city: "Chicago",
          jobprofile: "Project Manager",
          address: "789 Pine St, Chicago, IL"
        },
        {
          fname: "Emily",
          lname: "Davis",
          age: 22,
          city: "Houston",
          jobprofile: "Data Analyst",
          address: "101 Maple St, Houston, TX"
        },
        {
          fname: "David",
          lname: "Wilson",
          age: 30,
          city: "Phoenix",
          jobprofile: "Web Developer",
          address: "202 Oak St, Phoenix, AZ"
        },
        {
          fname: "Sarah",
          lname: "Brown",
          age: 26,
          city: "Philadelphia",
          jobprofile: "Content Writer",
          address: "303 Cedar St, Philadelphia, PA"
        },
        {
          fname: "James",
          lname: "Garcia",
          age: 38,
          city: "San Antonio",
          jobprofile: "Network Administrator",
          address: "404 Birch St, San Antonio, TX"
        },
        {
          fname: "Laura",
          lname: "Martinez",
          age: 29,
          city: "San Diego",
          jobprofile: "UX Designer",
          address: "505 Walnut St, San Diego, CA"
        },
        {
          fname: "Robert",
          lname: "Hernandez",
          age: 41,
          city: "Dallas",
          jobprofile: "Financial Analyst",
          address: "606 Chestnut St, Dallas, TX"
        },
        {
          fname: "Linda",
          lname: "Lee",
          age: 37,
          city: "San Jose",
          jobprofile: "Product Manager",
          address: "707 Spruce St, San Jose, CA"
        }
      ];
      
  return (
    <>
      <h1>Send Props Very Basic{baiscProps} </h1>
      <ArrayBasicProps sendArray={dummyData}/>
    </>
  )
}
